...
export async function recomputePlayerCareerStats(playerId) {
  // 1. All-time aggregate per format
  ...
  for (const s of formatStats) {
    ...
    const existing = await query(
      `SELECT id FROM career_stats WHERE player_id = ? AND format = ? AND season IS NULL`,
      [playerId, s.format]
    );
    ...
    if (existing.length > 0) {
      ...
      await query(
        `INSERT INTO career_stats 
         ...
         VALUES (?, ?, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          playerId, s.format, s.matches, runs, battingAvg, strikeRate,
